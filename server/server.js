...
io.on('connection', function (socket) {
  ...
  // Handle clearing the code editor for everyone in the room
  socket.on("clear editor", ({ roomId }) => {
    // Clear the code in the map for this room
    if (roomId in roomID_to_Code_Map) {
      roomID_to_Code_Map[roomId].code = ''; // Clear the code for this room
    }

    // Emit the event to all users in the room to clear their editors
    socket.to(roomId).emit("on editor clear");
  });
